...
export default function Clientes() {
  ...
  const inputClass = "w-full px-4 py-2.5 text-sm bg-neutral-50/60 border border-neutral-200/80 rounded-xl outline-none focus:ring-4 focus:ring-b2bHighlight/10 focus:border-b2bHighlight focus:bg-white transition-all placeholder:text-neutral-400 text-neutral-800";
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
        ...
        <div className="space-y-6 lg:col-span-1">
          ...
          <motion.div 
            ...
            className="bg-white p-6 rounded-2xl border border-neutral-200/50 shadow-premium-sm hover:shadow-premium-md transition-all duration-300"
          >
            <div className="flex items-center gap-3 mb-6 pb-4 border-b border-neutral-100">
              <div className="p-2 bg-emerald-50 rounded-lg text-b2bHighlight">
                <Plus size={16} />
              </div>
              <h2 className="text-xs font-black text-neutral-900 uppercase tracking-widest">1. Perfil de Cliente</h2>
            </div>
            <form onSubmit={registrarCliente} className="space-y-4">
              ...
              <motion.button 
                ...
                whileTap={{ scale: 0.99 }}
                type="submit" 
                className="w-full py-3 bg-neutral-900 text-white text-xs font-bold uppercase tracking-widest rounded-xl hover:bg-black transition-all shadow-premium-sm"
              >
                Dar de Alta
              </motion.button>
            </form>
          ...
          >
            <div className="flex items-center gap-3 mb-6 pb-4 border-b border-neutral-100">
              ...
              <h2 className="text-xs font-black text-neutral-900 uppercase tracking-widest">2. Llave de Acceso</h2>
            </div>
            <form onSubmit={registrarUsuario} className="space-y-4">
              ...
              <motion.button 
                ...
                className="w-full py-3 bg-neutral-900 text-white text-xs font-bold uppercase tracking-widest rounded-xl hover:bg-black transition-all shadow-premium-sm disabled:opacity-50 disabled:cursor-not-allowed"
              >
                Generar Acceso
              </motion.button>
            </form>
        ...
        <motion.div 
          variants={itemVariants}
          className="bg-white rounded-2xl border border-neutral-200/50 shadow-premium-sm lg:col-span-2 overflow-hidden hover:shadow-premium-md transition-all duration-300 h-full flex flex-col min-h-[500px]"
        >
          <div className="p-6 border-b border-neutral-100 flex justify-between items-center bg-neutral-50/40 shrink-0">
            <div className="flex items-center gap-3 text-neutral-500">
              <Building2 size={18} />
              <h2 className="text-sm font-bold text-neutral-900 tracking-tight">Directorio Maestro B2B</h2>
            </div>
            <span className="text-[9px] font-bold text-neutral-400 uppercase tracking-widest bg-white px-3 py-1.5 rounded-lg border border-neutral-200/50 shadow-premium-sm">
          ...
          {Array.isArray(clientes) && clientes.length > 0 ? (
              ...
              <table className="w-full text-left">
                ...
                <tbody className="divide-y divide-neutral-100/50">
                  {clientes.map(cliente => (
                    <tr key={cliente.id} className="hover:bg-neutral-50/30 transition-premium cursor-pointer group">
                      <td className="px-6 py-5">
                        <div className="flex items-center gap-3.5">
                          <div className="w-8.5 h-8.5 rounded-xl bg-neutral-50 border border-neutral-200/50 flex items-center justify-center text-neutral-400 group-hover:bg-b2bHighlight group-hover:text-white group-hover:border-b2bHighlight transition-all duration-200 shadow-premium-sm">
                            <Briefcase size={15}/>
                          </div>
                          <span className="text-sm font-semibold text-neutral-800 transition-colors group-hover:text-neutral-950">{cliente.nombre_comercial}</span>
                        </div>
                      </td>
                      <td className="px-6 py-5 text-sm font-mono text-neutral-500">{cliente.rfc}</td>
                      <td className="px-6 py-5">
                        <span className={`px-2.5 py-1 rounded-lg text-[9px] font-bold uppercase tracking-wider ${
                          cliente.plan_suscripcion === 'Premium' 
                            ...
                            : 'bg-neutral-100 text-neutral-600 border border-neutral-200/10'
                        }`}>
                          {cliente.plan_suscripcion}
                        </span>
                      </td>
                      <td className="px-6 py-5 text-center text-xs font-bold text-neutral-300 group-hover:text-neutral-400 transition-colors">#{cliente.id}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            <div className="flex-1 flex flex-col items-center justify-center p-12 text-center my-auto">
              <div className="w-16 h-16 bg-neutral-50 border border-neutral-200/60 rounded-2xl flex items-center justify-center text-neutral-400 mb-5 shadow-premium-sm animate-pulse">
                <Building2 size={28} />
              </div>
              <h3 className="text-sm font-bold text-neutral-800 mb-1">Directorio vacío</h3>
