...
export default function Configuracion() {
  ...
  const planes = [
    ...
    { 
      ...
      actual: userRol !== 'SuperAdmin' // Solo marcamos actual para clientes
    },
    { 
  ...
  const manejarTicket = (e) => {
    ...
    // Simula envío de ticket y luego cierra el modal
    setTimeout(() => {
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-10 animate-fade-in pb-20">
      ...
      {/* 1. PESTAÑA PERFIL (LITE) */}
      {tabActiva === 'perfil' && (
        <div className="bg-white p-12 rounded-3xl border border-neutral-200 shadow-sm text-center max-w-2xl mx-auto">
            ...
            <div className="mt-10 flex justify-center gap-4">
               <button className="px-6 py-2.5 bg-neutral-900 text-white rounded-xl text-xs font-black uppercase tracking-widest hover:bg-black transition-all">Cambiar Password</button>
               <button onClick={() => setSoporteAbierto(true)} className="px-6 py-2.5 bg-white border border-neutral-200 text-neutral-600 rounded-xl text-xs font-black uppercase tracking-widest hover:bg-neutral-50 transition-all">Soporte</button>
            </div>
        </div>
      )}

      {/* 2. PESTAÑA PLANES (GLOBAL - VISIBLE PARA TODOS) */}
      {tabActiva === 'planes' && (
        <div className="space-y-12">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {planes.map((plan) => (
              <div key={plan.nombre} className={`p-8 rounded-3xl border transition-all flex flex-col ${plan.actual ? 'bg-white border-b2bHighlight shadow-xl shadow-b2bHighlight/5 ring-1 ring-b2bHighlight' : 'bg-neutral-50 border-neutral-200 shadow-sm hover:border-neutral-300'}`}>
                <div className="mb-6">
                ...
                <div className="flex gap-2 w-full mt-auto">
                  <button className={`flex-1 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${plan.actual ? 'bg-b2bHighlight text-white hover:bg-emerald-500 shadow-lg shadow-emerald-100' : 'bg-white border border-neutral-200 text-neutral-400 hover:text-neutral-900 hover:border-neutral-900'}`}>
                    {plan.actual ? 'Gestionar' : 'Seleccionar Plan'}
                  </button>
                  {plan.actual && (
                    <button onClick={() => setSoporteAbierto(true)} className="px-4 bg-white border border-neutral-200 text-neutral-500 rounded-xl hover:bg-neutral-50 transition-colors">
                       <HelpCircle size={16} />
                    </button>
                  )}
                </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left">
                ...
                <tbody className="divide-y divide-neutral-50 text-[11px] font-medium text-neutral-600">
                  <tr><td className="px-8 py-4 font-bold text-neutral-900">Límite de Transacciones</td><td className="px-8 py-4 text-center">500 / mes</td><td className="px-8 py-4 text-center bg-b2bHighlight/5 font-black text-neutral-900 text-xs">Ilimitado</td><td className="px-8 py-4 text-center italic">Personalizado</td></tr>
                  <tr><td className="px-8 py-4 font-bold text-neutral-900">Usuarios Administradores</td><td className="px-8 py-4 text-center">1 Usuario</td><td className="px-8 py-4 text-center bg-b2bHighlight/5">Hasta 5 Usuarios</td><td className="px-8 py-4 text-center font-bold text-neutral-900">Ilimitados</td></tr>
                  <tr><td className="px-8 py-4 font-bold text-neutral-900">Retención de Auditoría</td><td className="px-8 py-4 text-center text-neutral-400">7 días</td><td className="px-8 py-4 text-center bg-b2bHighlight/5">180 días</td><td className="px-8 py-4 text-center">Historial Infinito</td></tr>
                  <tr><td className="px-8 py-4 font-bold text-neutral-900">Formatos de Exportación</td><td className="px-8 py-4 text-center">Sólo CSV</td><td className="px-8 py-4 text-center bg-b2bHighlight/5 font-bold text-neutral-800">CSV, Excel & PDF</td><td className="px-8 py-4 text-center">Todo + JSON API</td></tr>
                  <tr><td className="px-8 py-4 font-bold text-neutral-900">Inteligencia de Datos</td><td className="px-8 py-4 text-center">Básica</td><td className="px-8 py-4 text-center bg-b2bHighlight/5">Avanzada (KPIs)</td><td className="px-8 py-4 text-center font-bold text-indigo-600">IA Predictiva</td></tr>
                  <tr><td className="px-8 py-4 font-bold text-neutral-900">Acceso a API B2B</td><td className="px-8 py-4 text-center text-neutral-300">No disponible</td><td className="px-8 py-4 text-center bg-b2bHighlight/5">Sólo Lectura</td><td className="px-8 py-4 text-center font-bold text-neutral-900">Acceso Total</td></tr>
                  <tr><td className="px-8 py-4 font-bold text-neutral-900">Soporte Técnico</td><td className="px-8 py-4 text-center text-neutral-400">Email (48h)</td><td className="px-8 py-4 text-center bg-b2bHighlight/5">Chat Prioritario</td><td className="px-8 py-4 text-center font-bold text-indigo-600">Manager 24/7</td></tr>
                </tbody>
              </table>
            </div>
          </div>
      ...
      {/* 3. PESTAÑA ADMIN (SOLO SUPERADMIN) */}
      {tabActiva === 'admin' && userRol === 'SuperAdmin' && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <button onClick={() => navigate('/dashboard')} className="group bg-white p-8 rounded-2xl border border-neutral-200 shadow-sm hover:border-b2bHighlight transition-all text-left relative overflow-hidden">
          ...
          <button onClick={() => navigate('/clientes')} className="group bg-white p-8 rounded-2xl border border-neutral-200 shadow-sm hover:border-b2bHighlight transition-all text-left relative overflow-hidden">
      ...
      {soporteAbierto && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
          <div className="absolute inset-0 bg-neutral-900/40 backdrop-blur-sm transition-opacity" onClick={() => setSoporteAbierto(false)}></div>
          
          <div className="relative bg-white rounded-3xl shadow-2xl w-full max-w-3xl overflow-hidden animate-fade-in flex flex-col md:flex-row border border-neutral-100">
            
            {/* Columna Izquierda: FAQ (Oculta en móviles pequeños) */}
            <div className="bg-neutral-50 p-8 md:w-2/5 border-r border-neutral-100 hidden md:flex flex-col justify-between">
              <div>
                ...
                <div className="space-y-6">
                  <div>
                    <h4 className="text-sm font-bold text-neutral-800 mb-1">¿Sincronización de flujos?</h4>
                    <p className="text-xs text-neutral-500 leading-relaxed font-medium">Las métricas se procesan en tiempo real tras cada captura.</p>
                  </div>
                  <div>
                    <h4 className="text-sm font-bold text-neutral-800 mb-1">¿Cambios en el plan?</h4>
                    <p className="text-xs text-neutral-500 leading-relaxed font-medium">El upgrade es automático; el downgrade aplica al siguiente ciclo.</p>
                  </div>
                  <div>
                    <h4 className="text-sm font-bold text-neutral-800 mb-1">¿Datos en Excel?</h4>
                    <p className="text-xs text-neutral-500 leading-relaxed font-medium">Descarga tus reportes estructurados (.xlsx) desde el Dashboard.</p>
                  </div>
                </div>
              </div>
            ...
            <div className="p-8 md:w-3/5 bg-white">
              <div className="flex justify-between items-start mb-8">
                ...
                <button onClick={() => setSoporteAbierto(false)} className="p-2 bg-neutral-100 text-neutral-400 hover:bg-neutral-200 hover:text-neutral-900 rounded-full transition-all"><X size={18} /></button>
              </div>

              {ticketEnviado ? (
                <div className="flex flex-col items-center justify-center h-48 text-center space-y-4 animate-fade-in">
                ...
                <form onSubmit={manejarTicket} className="space-y-5 animate-fade-in">
                  <div>
                  ...
                  <button type="submit" className="w-full py-3.5 bg-neutral-900 hover:bg-black text-white text-xs font-black uppercase tracking-widest rounded-xl transition-all shadow-xl shadow-neutral-200 flex items-center justify-center gap-2 mt-4">
